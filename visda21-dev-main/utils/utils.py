...
def get_model_mme(net, num_class=13, temp=0.05, top=False, norm=True, cds=False, cds_path=None):
    dim = 2048
    if "resnet" in net:
        model_g = ResBase(net, top=top, cds=cds, cds_path=cds_path)
        if "resnet18" in net:
            dim = 512
        if net == "resnet34":
            dim = 512
        ...
        model_g = VoloBase(option=net, pret=True, feat=True)
        if "volod3" in net:
            dim = 512
    if top:
        dim = 1000
...
def save_model(model_g, model_c1, model_c2, model_d, save_path):
    save_dic = {
